...
export default async function DomainsPage(){
  ...
  return <main className="wrap">
    ...
    <section className="card">
      ...
      {!state.enterpriseFeatures&&<div className="notice">Custom domains are available on custom/Enterprise accounts. Contact Mkety to enable this feature for your account.</div>}
      {state.enterpriseFeatures&&<form method="post" action="/api/domains/request" className="form">
        ...
        <p className="muted">Use a subdomain you control. Mkety provisions TLS; you will only need to add the DNS record shown below.</p>
      </form>}
    </section>
    ...
    {(domains.results||[]).map((d:any)=><section className="card" style={{marginTop:18}} key={d.id}>
      ...
      {d.ownership_name&&d.ownership_value&&<div className="notice"><p>If ownership validation is requested, add:</p><p><strong>{d.ownership_type||"TXT"}</strong> {d.ownership_name} → {d.ownership_value}</p></div>}
