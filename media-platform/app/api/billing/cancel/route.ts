...
export async function POST(request:Request){
  ...
  const pending=await db.prepare("SELECT id,payment_method FROM media_invoices WHERE tenant_id=? AND status='pending' ORDER BY created_at DESC LIMIT 1").bind(user.tenantId).first<any>();
  if(pending&&String(pending.payment_method||"invoice")!=="invoice"){
