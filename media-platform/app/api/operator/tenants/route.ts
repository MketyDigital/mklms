...
export async function POST(request:Request){
  ...
  await db.batch([
    db.prepare("UPDATE media_tenants SET status=? WHERE id=?").bind(status,tenantId),
    db.prepare(
      "INSERT INTO media_tenant_commercial_terms (tenant_id,base_plan_code,display_name,monthly_usd,storage_bytes,delivery_bytes,delivery_requests,logical_buckets,team_seats,max_object_bytes,overage_mode,enterprise_features,infrastructure_mode,billing_term_months,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,datetime('now')) ON CONFLICT(tenant_id) DO UPDATE SET display_name=excluded.display_name,monthly_usd=excluded.monthly_usd,storage_bytes=excluded.storage_bytes,delivery_bytes=excluded.delivery_bytes,delivery_requests=excluded.delivery_requests,logical_buckets=excluded.logical_buckets,team_seats=excluded.team_seats,max_object_bytes=excluded.max_object_bytes,overage_mode=excluded.overage_mode,enterprise_features=excluded.enterprise_features,infrastructure_mode=excluded.infrastructure_mode,billing_term_months=excluded.billing_term_months,updated_at=datetime('now')"
    ).bind(
      tenantId,
