...
export default function TrustPage(){
  const rows=[
    ...
    ["Backups & retention","Standard plans include infrastructure redundancy, not a separate point-in-time backup service. Custom customers can request retention/deletion-protection or separate backup/migration arrangements as part of their order."],
    ["Availability","The standard storage layer is backed by infrastructure with a 99.9% availability SLA. Mkety does not advertise a separate contractual customer SLA unless it is included in an Custom agreement."],
    ...
    ["Public vs private files","Standard Mkety Media delivery URLs are public: anyone who has the URL can request the file. Do not use standard public buckets for secrets or access-controlled documents. Private/signed delivery can be discussed as an Custom requirement."],
    ["Bulk export","Every customer can export the full library inventory and permanent URLs as JSON/CSV, or download scripts for macOS/Linux and Windows. Custom customers can request assisted migrations."],
    ["Custom domains","Custom/Custom accounts can request a branded hostname such as media.example.com. Mkety provisions the hostname/TLS and the customer points a CNAME to the Mkety delivery target."],
    ...
    ["Deletion","Deleting a file removes the source object and requests cache invalidation. Standard delivery caching uses a bounded edge TTL; customers needing formal retention/deletion controls should use an Custom policy."],
    ...
    ["Data residency","Standard storage placement is managed by Mkety's infrastructure layer. A specific country/region residency commitment is only provided when explicitly agreed for an Custom deployment."],
  ];
