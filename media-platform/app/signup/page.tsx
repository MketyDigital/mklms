    ...
    <main className="wrap">
      <form className="card form" method="post" action="/api/auth/signup">
        ...
        <select name="plan" defaultValue={params.plan||"starter"}>
          {plans.map((plan:any)=><option key={plan.code} value={plan.code}>{plan.name} — {"$"}{Number(plan.monthly_usd).toFixed(0)}/mo</option>)}
        </select>
        <label>Billing term</label>
        <select name="term" defaultValue="1">
          {(terms as any[]).map((term)=><option key={term.months} value={term.months}>{term.label}{Number(term.discountPercent||0)>0?" — "+Number(term.discountPercent)+"% off":""}</option>)}
        </select>
