...
import { FixedWindowRateLimiter, getRequestClientKey, rateLimitHeaders } from "@/lib/security/rate-limit";
...
export async function POST(request: Request) {
  ...
  if (!limit.allowed) {
    return NextResponse.json({ ok: false, message: "Too many sign-in attempts. Try again later." }, { status: 429, headers: rateLimitHeaders(limit) });
  }
