...
export function AdminCourseManager({ initialCourses }: AdminCourseManagerProps) {
  ...
  async function editCourse(course: CourseRecord) {
    ...
    try {
      const response = await fetch(`/api/admin/courses/${course.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title: nextTitle, description: nextDescription || null }),
      });
      const result = await response.json().catch(() => null);
      if (!response.ok || !result?.ok) {
          ...
          <Card key={course.id}>
            <CardContent className="flex flex-col gap-4 py-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button variant="outline" size="sm" disabled={busy} onClick={() => void editCourse(course)}>Edit course</Button>
