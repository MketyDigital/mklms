...
export function AdminAccessPanel({ initialStudents, initialPreauthorizations, courses, defaultClaimStrategy }: AdminAccessPanelProps) {
  ...
  async function addStudent(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(data.preauthorization?.created === false ? "This identity is already authorized for that course." : "Student pre-authorized successfully.");
  ...
  async function bulkAuthorize(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const codes = (data.created ?? []).filter((row: { claimCode?: string }) => row.claimCode).map((row: { email?: string; phone?: string; claimCode: string }) => `${row.email ?? row.phone ?? "Student"}: ${row.claimCode}`);
