...
export class PostgresAdminLiveClassRepository implements AdminLiveClassRepository {
  ...
  async listBatches(): Promise<AdminLiveBatchRecord[]> {
    const result = await this.pool.query<any>(`SELECT id, slug, title, description, status, expected_viewer_baseline,
  ...
  async createBatch(input: Omit<AdminLiveBatchRecord, "id">): Promise<AdminLiveBatchRecord> {
    const result = await this.pool.query<any>(`INSERT INTO live_batches (
  ...
  async listSessions(batchId: string): Promise<AdminLiveSessionRecord[]> {
    const result = await this.pool.query<any>(`SELECT id,batch_id,title,position,starts_at,duration_seconds,media_asset_id,status,
  ...
  async createSession(batchId: string, input: Omit<AdminLiveSessionRecord, "id" | "batchId">): Promise<AdminLiveSessionRecord> {
    const result = await this.pool.query<any>(`INSERT INTO live_sessions (
  ...
  private mapSession(row: any): AdminLiveSessionRecord {
  ...
  private mapBatch(row: any): AdminLiveBatchRecord {
