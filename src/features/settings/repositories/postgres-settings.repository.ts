...
export class PostgresSettingsRepository implements SettingsRepository {
  ...
  async getPlatformSettings(): Promise<PlatformSettings> {
    const result = await this.pool.query<{
      ...
      timezone: string;
      locale: string;
      access_provider: PlatformSettings["accessProvider"];
      claim_verification_strategy: PlatformSettings["claimVerificationStrategy"];
      storage_provider: PlatformSettings["storageProvider"];
      media_provider: PlatformSettings["mediaProvider"];
      email_provider: PlatformSettings["emailProvider"];
      notification_provider: PlatformSettings["notificationProvider"];
  ...
  async savePlatformSettings(settings: PlatformSettings): Promise<void> {
    await this.pool.query(
      `INSERT INTO platform_settings (
         id, organization_name, product_name, logo_url, favicon_url,
         primary_color, secondary_color, support_name, support_email,
         public_base_url, timezone, locale, access_provider,
         claim_verification_strategy, storage_provider, media_provider,
         email_provider, notification_provider, access_code_prefix,
       ...
       ON CONFLICT (id) DO UPDATE SET
         ...
         claim_verification_strategy = EXCLUDED.claim_verification_strategy,
         storage_provider = EXCLUDED.storage_provider,
         media_provider = EXCLUDED.media_provider,
         email_provider = EXCLUDED.email_provider,
         notification_provider = EXCLUDED.notification_provider,
         access_code_prefix = EXCLUDED.access_code_prefix,
