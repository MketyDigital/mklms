...
For the current setup `STARPIPS_DATABASE_SSL` should preserve the existing SSL mode; `require` is the normal value when the existing connection uses required TLS.
...
The workflow checks only whether required repository secrets are non-empty and whether release workflows use the Free/private-compatible secret contract. It never prints secret values.

Do not make the repository private until this workflow is green.
...
4. Run `Verify GitHub Free private readiness` and require success.
5. Change repository visibility to **Private**.
6. Do not move either production pointer as part of the visibility change.
7. Run the normal CI workflow from a harmless development/docs change, or manually run the security hardening verification, to prove private-repository Actions execute successfully.
8. Run `Cloudflare read-only verify` for Starpips and Mkety Academy when desired; this is GET-only and must not mutate production.
9. Keep the existing Cloudflare deployments untouched unless there is an actual application release to promote.
...
Private-repository GitHub-hosted Actions consume the plan's included Actions allowance. CI in this repository performs Node dependency installation, tests, Next.js build, OpenNext build, and three Worker packaging checks, so repeated runs can consume minutes faster than a small repository.
...
- use concurrency/cancel behavior where safe for non-production CI;
